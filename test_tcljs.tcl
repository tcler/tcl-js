...
proc e.g. {cmd -> expected} {
    ...
    if ![string equal $mres $expected] {
	puts "**** $cmd -> $mres, expected $expected"
    } else {incr ::passed; incr ::fail -1}
}

#set noTcl ![info exists auto_path] ;# testing tcl.js or real Tcl?
...
e.g. {f 41} -> {x y} ;# throws exception - must fix proc call in uplevel issue
set tmp [f 41]; e.g. {set tmp} -> {x y} ;# works: proc call and uplevel decoupled
e.g. {info args f}      -> x
